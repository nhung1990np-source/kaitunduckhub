--[========================================================]
-- FILE: maru_evolution_v7_nousigi_ui_v4.lua
-- VERSION: v7-nousigi-ui-v4-square-toggles
-- UPDATED: 2026-09-16
...
--      -> setclipboard copy link https://discord.gg/https://discord.gg/faXyeKXv7C
--
...
-- [MERGED FROM FILE 1] Resolve quest NPC and mob spawn positions dynamically.
-- This keeps file 2's UI, tween, attack and farm loops unchanged while avoiding stale hard-coded coordinates.
do
...
local function GetQuestPointForQuest(questName)
    if not questName or not GuideModule or not GuideModule.Data or not GuideModule.Data.NPCList then
        return nil
    end

    for npc, data in pairs(GuideModule.Data.NPCList) do
        if data and data.InternalQuestName == questName then
            local position = ToCFrame(data.Position)
...
local function FindMobSpawn(mobName)
    if not mobName then return nil end
    local wanted = NormalizeMobName(mobName)

    if workspace:FindFirstChild("Enemies") then
        for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
            local root = enemy:FindFirstChild("HumanoidRootPart")
            local humanoid = enemy:FindFirstChildOfClass("Humanoid")
            if NormalizeMobName(enemy.Name) == wanted and root and humanoid and humanoid.Health > 0 then
                return root.CFrame
            end
        end
    end
    ...
    if spawns then
        for _, spawnPoint in ipairs(spawns:GetChildren()) do
            if spawnPoint:IsA("BasePart") and NormalizeMobName(spawnPoint.Name) == wanted then
                return spawnPoint.CFrame
            end
        end
        for _, spawnPoint in ipairs(spawns:GetChildren()) do
            if spawnPoint:IsA("BasePart") and string.find(NormalizeMobName(spawnPoint.Name), wanted, 1, true) then
                return spawnPoint.CFrame
            end
        end
    end
...
GetQuestPoint = function(mobName)
    ...
    if questData and (not mobName or questData.Mob == mobName) then
        local point = GetQuestPointForQuest(questData.NameQuest)
...
GetMobFarmPosition = function(mobName)
    return FindMobSpawn(mobName)
end

GetBoneQuestPosition = function()
    return GetQuestPointForQuest("HauntedQuest2")
        or GetQuestPointForQuest("HauntedQuest1")
        or CFrame.new(-9509.34961, 142.130661, 5535.16309)
end

GetBoneFarmPosition = function(mobNames)
    for _, mobName in ipairs(mobNames or {}) do
        local point = FindMobSpawn(mobName)
        if point then return point end
    end
    return CFrame.new(-9509.34961, 142.130661, 5535.16309)
end
end
...
return function(Library)
    ...
    local nativeWindow = Library:CreateWindow({
        Title = "DUCK HUB",
        Desc = "by DUCZ ",
        Image = "rbxassetid://110438945732939"
    })
...
local T1UIColor = {
	...
	["Logo Image"] = "rbxassetid://110438945732939",
...
local libCreateNoti = function(Setting)
	...
	CloseImage.Image = "rbxassetid://110438945732939"
...
function Library:CreateWindow(Setting)
	...
	maingui.Image = "rbxassetid://110438945732939"
	...
	SearchIcon.Image = "rbxassetid://110438945732939"
	...
	Shadow.ZIndex = 0
	Shadow.Image = "rbxassetid://110438945732939"
	...
	function Main_Function:AddTab(PageName)
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			LineShadow.ZIndex = 0
			LineShadow.Image = "rbxassetid://110438945732939"
			...
			if Toggleable then
				...
				visibility.ZIndex = 2
				visibility.Image = "rbxassetid://110438945732939"
				...
				visibility_off.Image = "rbxassetid://110438945732939"
			...
			function sectionFunction:AddToggle(idk,Setting)
				...
				checkbox.BackgroundColor3 = Color3.fromRGB(42, 42, 42)
				...
				checkboxStroke.Color = Color3.fromRGB(255, 206, 27)
        ...
        function sectionFunction:AddButton(Setting, Callback)
            ...
            TextColor_1.Parent = RowBG_1
             ...
             ImageLabel_1.Image = "rbxassetid://110438945732939"
            ...
            function sectionFunction:AddDropdownSection(Setting)
                ...
                ImgDrop.Image = "rbxassetid://110438945732939"
				...
				local function refreshlist(SortPairs)
					...
					if Selected then
                        for _, i in ipairs(OrderedList) do
							...
							SampleItemCheck.Image = "rbxassetid://110438945732939"
...
Tabs.Info:AddParagraph({Title = "DUCK HUB", Content = "Hongs UPD and KAITUN NEW" .. "\nInvite: " .. "https://discord.gg/faXyeKXv7C"})
...
Tabs.Info:AddButton({
    ...
    Callback = function()
        local discordLink = "https://discord.gg/faXyeKXv7C"
        if setclipboard then
...
-- Mỗi chế độ farm dùng một công tắc vuông. Bật chế độ mới sẽ tự tắt chế độ cũ.
SetFarmToggleMode = function(mode, enabled, optionKey)
    if enabled then
        _G.SelectedFarmMode = mode
        _G.Level = mode == "Farm Level"
        _G.Auto_Cake_Prince = mode == "Farm Cake Prince"
        _G.FarmTyrant = mode == "Farm Tyrant of Skies"
        _G.AutoFarm_Bone = mode == "Farm Bone"
        _G.ActiveFarm = ({["Farm Level"] = "Level", ["Farm Cake Prince"] = "CakePrince", ["Farm Tyrant of Skies"] = "Tyrant", ["Farm Bone"] = "Bone"})[mode]
        alreadyTeleported = false
        teleporting = false
        for _, key in ipairs({"Main_AutoFarmLevel", "Main_AutoFarmCakePrince", "Main_AutoFarmTyrant", "Main_AutoFarmBone"}) do
            if key ~= optionKey and Fluent.Options[key] and Fluent.Options[key].Value then
                Fluent.Options[key]:SetValue(false)
            end
        end
    else
        if optionKey == "Main_AutoFarmLevel" then _G.Level = false end
        if optionKey == "Main_AutoFarmCakePrince" then _G.Auto_Cake_Prince = false end
        if optionKey == "Main_AutoFarmTyrant" then _G.FarmTyrant = false end
        if optionKey == "Main_AutoFarmBone" then _G.AutoFarm_Bone = false end
        if _G.SelectedFarmMode == mode then
            _G.SelectedFarmMode = "None"
            _G.ActiveFarm = nil
        end
    end
end

Tabs.Main:AddToggle("Main_AutoFarmLevel", {
    Title = "Auto Farm Level",
    Description = "Tự nhận quest và farm theo cấp độ tài khoản",
    Default = false,
    Callback = function(Value)
        SetFarmToggleMode("Farm Level", Value, "Main_AutoFarmLevel")
        if Value then
...
Tabs.Main:AddToggle("Main_AutoFarmCakePrince", {
    Title = "Auto Farm Cake Prince",
    Description = "Tự động farm Cake Prince",
    Default = false,
    Callback = function(Value)
        SetFarmToggleMode("Farm Cake Prince", Value, "Main_AutoFarmCakePrince")
    end
})

Tabs.Main:AddToggle("Main_AutoFarmTyrant", {
    Title = "Auto Farm Tyrant of Skies",
    Description = "Tự động farm Tyrant of the Skies",
    Default = false,
    Callback = function(Value)
        SetFarmToggleMode("Farm Tyrant of Skies", Value, "Main_AutoFarmTyrant")
    end
})

Tabs.Main:AddToggle("Main_AutoFarmBone", {
    Title = "Auto Farm Bones",
    Description = "Tự động farm Bones",
    Default = false,
    Callback = function(Value)
        SetFarmToggleMode("Farm Bone", Value, "Main_AutoFarmBone")
    end
})
...
-- Auto Farm Level toggle đã được đặt phía trên cùng các chế độ farm khác
...
-- Công tắc Auto Farm Bones đã được đặt trong mục Farming phía trên.
    ...
    while wait(0.5) do
        ...
        pcall(function()
            ...
            if _G.AcceptQuestB and questUI and not questUI.Visible then
                local questPos = GetBoneQuestPosition()
            ...
            if bone then
                ...
                local boneSpawn = GetBoneFarmPosition(BonesTable)
                if boneSpawn then _tp(boneSpawn * CFrame.new(0, 60, 0)) end
            end
        end)
    end
end)
...
do
    ...
    Callback =  function()
  for i, v in pairs(game:GetObjects("rbxassetid://110438945732939")[1]:GetChildren()) do
